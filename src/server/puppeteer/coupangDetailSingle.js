const CoupangItem = require("../models/CoupangItem")
...
const start = async ({ url }) => {
  try {
    ...
    if (response && response.options) {
      ...
      if (optionRows.length === 1) {
        optionRows[0].attributes.forEach(attItem => {
          ...
          let diff = null
          let shippingFee = 0
          if (attributeVendorItemMap[attItem.valueId]) {
            ...
            if (attributeVendorItemMap[attItem.valueId].quantityBase[0].delivery.descriptions) {
              ...
              originImage = `https:${attributeVendorItemMap[attItem.valueId].images[0].origin}`
    ...
    const coupangItem = await CoupangItem.findOneAndUpdate(
      {
    ...
    return coupangItem._id
  } catch (e) {
    console.log("coupangDetailSingle", e)
  }
}
