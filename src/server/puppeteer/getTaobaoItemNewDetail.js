...
const { TaobaoDetailOption, TaobaoDetailImage, TMallOptionApi, ItemSKU } = require("../api/Taobao")
...
const start = async ({ url, userID }) => {
  ...
  try {
    ...
    const promiseArr = [
      new Promise(async (resolve, reject) => {
        try {
          ...
          if (response && response.status === 200) {
            ...
            const taobaoHtml = response.data.toString()
            console.log("성공 --- ", taobaoHtml)
            ...
            const {
              ...
              originalPrice,
              promotionPrice,
            } = await getBasicInfo({ url, taobaoHtml, cookie: cookies[0].cookie })
          ...
          ObjItem.salePrice = ObjItem.options[0] ? ObjItem.options[0].price : 0
      ...
      new Promise(async (resolve, reject) => {
        try {
          const { afterServiceInformation, afterServiceContactNumber, topImage, bottomImage } =
    ...
    logger.error(`taobaoDetailNew, ${e}`)
    return null
  } finally {
    if (page) {
...
const getBasicInfo = async ({ url, taobaoHtml, cookie }) => {
  const obj = {}
  try {
    if (/item.taobao.com/.test(url)) {
      // 타오바오
      console.log("taobaoHtml", typeof taobaoHtml)
      console.log("taobaoHtml", taobaoHtml)
      obj.itemId = taobaoHtml.split("itemId           : '")[1].split("',")[0].trim()
      obj.sellerId = taobaoHtml.split("sellerId         : '")[1].split("',")[0].trim()
      obj.shopId = taobaoHtml.split("shopId           : '")[1].split("',")[0].trim()
      const auctionImage = taobaoHtml.split("auctionImages    : ")[1].split("},")[0].trim()
      ...
      const detailImageUrl = `http:${
        taobaoHtml.split("descUrl          : ")[1].split("' ? '")[1].split("' : '")[0]
      }`
      ...
      console.log("taobaoOtpion --- ", taobaoOtpion)
      parserReponse = JSON.parse(taobaoOtpion)

      console.log("parserReponse", parserReponse.code)
      // parserReponse = JSON.parse(
      //   taobaoOtpion.replace("onSibRequestSuccess(", "").replace(")", "").replace(/;/gi, "")
      // )
    ...
    } else if (/detail.tmall.com/.test(url)) {
      // 티몰
      const tshopSetup = taobaoHtml.split("TShop.Setup(")[1].split(");")[0].trim()
      // console.log("tshopSetup", tshopSetup)
      ...
      let optionResponse = await TMallOptionApi({
        path: initApi,
    ...
    if (!skuQuantity) {
      ...
      if (response.prop) {
        ...
        for (const sku of sku_base.skus) {
          ...
          for (const path of propPathArr) {
            if (path.split(":").length === 2) {
              ...
              const propsValue = response.prop
                .filter((item) => item.pid === pid)[0]
                .values.filter((item) => item.vid === vid)[0]
          ...
          tempOption.push({
            ...
            price: skus[sku.skuId].promotion_price
              ? skus[sku.skuId].promotion_price
              : skus[sku.skuId].price,
        ...
        tempOption.push({
          ...
          value: "单一商品",
          korValue: "단일상품",
          image: `https:${response.item.pic}`,
          price: response.item.promotion_price
            ? response.item.promotion_price
            : response.item.price,
          stock: response.item.quantity,
          disabled: false,
          active: true,
        })
      }
    ...
    const options = await page.$$eval(".J_TSaleProp", (elem) => {
      ...
      for (let rootIndex = 0; rootIndex < elem.length; rootIndex++) {
        ...
        for (let index = 0; index < elem[rootIndex].querySelectorAll("ul > li").length; index++) {
          ...
          const value = elem[rootIndex]
            .querySelectorAll("ul > li")
            [index].querySelector("a > span").textContent
          ...
          if (style && style.includes("background:url")) {
            image = `http:${
              style.replace("background:url(", "").split("_30x30.jpg")[0].split("_40x40q90.jpg")[0]
            }`
          }
          optionArray.push({
            key,
...
const loginWindow = async (page, user) => {
  try {
    if (await page.$(".sufei-dialog-content > #sufei-dialog-content")) {
      await page.waitForSelector(".sufei-dialog-content > #sufei-dialog-content", { timeout: 5000 })
    }

    var frames = await page.frames()

    var myframe = frames.find((f) => f.url().indexOf("https://login.taobao.com/") > -1)
    ...
    if (myframe) {
      ...
      await myframe.waitForSelector("#fm-login-id")
      await myframe.tap("#fm-login-id")
      await myframe.type("#fm-login-id", accountInfo.loginID, opts)

      await myframe.tap("#fm-login-password")
      await myframe.type("#fm-login-password", accountInfo.password, opts)

      await page.waitFor(3000)
      await myframe.waitForSelector("#nocaptcha-password")
      const slider = await myframe.$eval("#nocaptcha-password", (node) => node.style)

      if (slider && Object.keys(slider).length) {
        await mouseSlide(myframe, page)
      }

      let loginBtn = await myframe.$(".fm-button.fm-submit.password-login")
      ...
      var myframe = frames.find((f) => f.url().indexOf("https://detailskip.taobao.com/") > -1)

      if (!myframe) {
        myframe = frames.find((f) => f.url().indexOf("https://rate.taobao.com/") > -1)
      }
...
const mouseSlide = async (frame, page) => {
  let bl = false
  while (!bl) {
    try {
