...
import { AmazonOutlined, CloseOutlined } from "@ant-design/icons"
import { getByte } from "../../../lib/userFunc"

const { shell } = window.require("electron")

const AmazonCollectionItem = forwardRef(({ collection, onDelete }, ref) => {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(collection.feature)) {
      if (collection.feature.length > 0) {
        tempHtml += `<ul>`
      }
      for (const item of collection.feature) {
        tempHtml += `<li>${item}</li>`
      }
      if (collection.feature.length > 0) {
        tempHtml += `</ul><br /><br />`
    ...
    setItem({
      ...
      options: collection.options.map((item) => {
        return {
          ...
          weightPrice: Number(
            collection.shippingWeightInfo.map((item) => item).sort((a, b) => a.title - b.title)[0]
              .price
          ),
...
export default AmazonCollectionItem
