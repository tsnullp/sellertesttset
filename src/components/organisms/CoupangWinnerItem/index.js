...
import { Divider, Tooltip, Input, Tag, Image, Button,Dropdown, Drawer, Menu, Checkbox, BackTop, notification, Spin } from "antd"
...
const CoupangWinnerItem = forwardRef(({ loading, count, list, subPrice }, ref, ) => {
  ...
  const setSubPrice = (index, subPrice) => { 
    setData(data.map((item, i) => {
      if(i === index){
...
export default CoupangWinnerItem
...
const CoupangItem = ({ isRegister, productId, vendorName, vendorID, ratingCount, ratingAveragePercentage,
  ...
  detailUrl, subPrice, isClothes, isShoes
}) => {
          ...
          <div>
            <Tooltip title="쿠팡 판매금액에서 차감할 금액을 입력해주세요.">
              <Input
                ...
                value={deductedAmount.toLocaleString("ko")}
                onChange={e => {
                  let value = Number(e.target.value.replace(/,/gi, ""))
                  SetDeductedAmount(value)
                }}
                onBlur={e => {
                  let value = Number(e.target.value.replace(/,/gi, ""))
                  if (value <= 100) {
                    SetDeductedAmount(100)
                    setRootShoes(index, 100)
                  } else {
                    SetDeductedAmount(value)
                    setRootShoes(index, value)
                  }
                }}
                onKeyPress={event => {
                  if (
                ...
                <div>
                  <div>
                    <Button
                      ...
                      onClick={() => {
                        SetDeductedAmount(subPrice + 100)
                        setRootSubPrice(index, subPrice + 100)
                      }}
                    >
                      <UpOutlined />
                    </Button>
                ...
                <div>
                  <div>
                    <Button
                      style={{ width: "40px", height: "24px", padding: "0" }}
                      onClick={() => {
                        SetDeductedAmount(subPrice + 1000)
                        setRootSubPrice(index, subPrice + 1000)
                      }}
                    >
                      <UpOutlined />
                    </Button>
