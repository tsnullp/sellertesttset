...
import { GET_ALIEXPRESS_DETAIL_API } from "../../../gql"
import { RedoOutlined, CloseOutlined, AmazonOutlined } from "@ant-design/icons"
import { getByte } from "../../../lib/userFunc"
import moment from "moment"
const { shell } = window.require("electron")

const AliUploadItem = forwardRef(({ asin, url, productName, onDelete, onComplete }, ref) => {
  ...
  const { error, refetch, networkStatus } = useQuery(GET_ALIEXPRESS_DETAIL_API, {
    variables: {
      url,
      title: productName,
    },
    ...
    onCompleted: (data) => {
      ...
      if (data.GetAliExpressDetailAPI && !data.GetAliExpressDetailAPI.isRegister) {
        let tempHtml = ``
        tempHtml += `<hr >`
        if (Array.isArray(data.GetAliExpressDetailAPI.content)) {
          for (const item of data.GetAliExpressDetailAPI.content) {
            tempHtml += `<img src="${item}" style="width: 100%; max-width: 800px; display: block; margin: 0 auto; "/ />`
          }
        }

        setItem({
          ...data.GetAliExpressDetailAPI,
          html: tempHtml,
          options: data.GetAliExpressDetailAPI.options.map((item) => {
            return {
              ...item,
              ...
              salePrice: getSalePrice(
                item.price,
                data.GetAliExpressDetailAPI.shippingWeightInfo
                  .map((item) => item)
                  .sort((a, b) => a.title - b.title),
                data.GetAliExpressDetailAPI.marginInfo,
                data.GetAliExpressDetailAPI.exchange
              ),
            }
          }),
        })
    ...
    return (
      <Container>
        <div>
          ...
          >
            <img
              src="//ae01.alicdn.com/images/eng/wholesale/icon/aliexpress.ico"
              width="16px"
