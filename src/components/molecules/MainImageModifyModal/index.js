...
import { UPLOAD_IMAGE, GET_MAIN_IMAGES, MODIFY_PRODUCT_MAINIMAGES } from "../../../gql"
import {arrayMoveImmutable} from "array-move"
import { SortableContainer, SortableElement } from "react-sortable-hoc"

const MainImageModifyModal = ({isModalVisible, handleOk, handleCancel, _id, user}) => {
  const [images, setImages] = useState([])
  const [modifyProductMainImages] = useMutation(MODIFY_PRODUCT_MAINIMAGES)
  // useEffect(() => {
  //   setImages(mainImages)
  // }, [mainImages])

  const {data, refetch} = useQuery(GET_MAIN_IMAGES, {
    variables: {
      _id
    },
    onCompleted: data=> {
      setImages(data.GetMainImages || [])
    }
  })
  if(images&&images.length === 0 && data && data.GetMainImages.length > 0){
    setImages(data.GetMainImages)
  }
  useEffect(() => {
    if(isModalVisible) {
  ...
  const handleOkButton = async() => {
    ...
    if(images.length > 0){
      ...
      const response = await modifyProductMainImages({
        variables: {
          id: _id,
          mainImages: images,
          userID: user
...
export default MainImageModifyModal
